...
public class OpenCSVTests {
    ...
    public void openCSVTestTimeLineByLineParsing() throws IOException {
    ...
    public void openCSVTestTimeLineByLineWithBufferParsing() throws IOException {
        List<Long> times = new LinkedList<>();
        int SIZE_OF_BUFFER = 8192;

        long attempt = 0;
        while (attempt <= iterationCount) {
            times.add(new OpenCSVParser(name).parseLineByLineWithBuffer(SIZE_OF_BUFFER));
            attempt++;
        }
